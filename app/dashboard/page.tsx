        ...
        <div>
          <div className="flex items-center justify-between mb-4">
            ...
            <div className="flex items-center gap-2">
              {satirlar.filter((s) => s.refSayisi > 0).length >= 2 && (
                <button
          ...
          {satirlar.length > 0 && (
            <div className="grid grid-cols-3 gap-3 mb-4">
              {[
                { label: "Toplam Link", value: satirlar.length, color: "text-white" },
                { label: "Bekleyen", value: satirlar.filter((s) => s.refSayisi === 0).length, color: "text-amber-400" },
                { label: "Tamamlanan", value: satirlar.filter((s) => s.refSayisi > 0).length, color: "text-emerald-400" },
              ].map((stat) => (
                  ...
                  >
                    ...
                    <div className="flex items-center gap-4">
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="text-white font-semibold text-sm mb-1 flex items-center gap-1.5 flex-wrap">
                        ...
                        <div className="flex items-center gap-1.5 mb-1.5">
                          <span className="text-xs text-slate-500 font-medium">Link Hazır</span>
                          <span className="text-slate-600 text-[11px] font-mono bg-white/5 border border-white/10 rounded px-1.5 py-0.5">
                            #{link.id.slice(-6)}
                          </span>
                        </div>
                      ...
                      {/* ── Üst sağ: sadece ikincil ikon butonlar ── */}
                              ...
                              <button
                                ...
                                className={`flex items-center justify-center w-8 h-8 rounded-lg transition-all border ${
                                  kopyalandi === link.id
                                    ...
                                    : "border-white/10 text-slate-500 hover:text-indigo-300 hover:border-indigo-500/30 hover:bg-indigo-500/10"
                                }`}
                              >
                                {kopyalandi === link.id ? <Check className="w-3.5 h-3.5" /> : <Copy className="w-3.5 h-3.5" />}
                              </button>
                              <span className="pointer-events-none absolute bottom-full left-1/2 -translate-x-1/2 mb-2 whitespace-nowrap rounded-lg bg-gray-900 px-2.5 py-1 text-[11px] font-medium text-white opacity-0 group-hover/tip:opacity-100 transition-opacity duration-150 shadow-lg">
                                {kopyalandi === link.id ? "Kopyalandı!" : "Linki Kopyala"}
                                <span className="absolute left-1/2 top-full -translate-x-1/2 border-4 border-transparent border-t-gray-900" />
                              </span>
                            </div>
                            {/* WhatsApp ikon — her zaman göster */}
                            <div className="relative group/tip">
                                <a
                                  href={`https://wa.me/?text=${encodeURIComponent(`Kiracı referans linkime göz atabilirsiniz: ${url}`)}`}
                                  target="_blank"
                                  rel="noopener noreferrer"
                                  className="flex items-center justify-center w-8 h-8 rounded-lg text-slate-500 hover:text-[#25D366] hover:bg-[#25D366]/10 border border-white/10 hover:border-[#25D366]/30 transition-all"
                                >
                      ...
                      <div className="mt-3 pt-3 border-t border-white/10 space-y-2">
                        ...
                        {refSayisi > 0 && (
                          <button
                            ...
                            className={`w-full flex items-center justify-center gap-2 text-sm font-bold px-4 py-2.5 rounded-xl transition-all border-2 disabled:opacity-60 ${
                              raporKopyalandi[link.id]
                                ? "bg-emerald-500/20 border-emerald-500/40 text-emerald-300"
                                : "bg-indigo-600 border-indigo-500 text-white hover:bg-indigo-500 hover:border-indigo-400 shadow-lg shadow-indigo-500/25"
                            }`}
                          >
                            {raporKopyalandi[link.id] ? (
                              ...
                              <><FileText className="w-4 h-4" />Rapor Linkini Kopyala<span className="text-xs font-normal opacity-80 ml-1">(Yeni ev sahibine gönder)</span></>
                            )}
                          </button>
                        )}
                      </div>
      ...
      >
        ...
        <div className="flex items-center gap-3 mb-5">
          ...
          <div>
            ...
            <p className="text-xs text-gray-400">Yeni ev sahibine göndereceğin genel profil linkini oluştur.</p>
            <p className="text-xs text-gray-500 font-medium mt-0.5">Seçtiğin referanslar bu linkte görünür. İstediğin zaman güncelleyebilirsin.</p>
          </div>
        </div>
      ...
      >
        ...
        <div className="flex items-center gap-3 mb-6">
          ...
          <div>
            ...
            <p className="text-xs text-gray-400">Eski ev sahibine göndereceğin referans linkini oluştur.</p>
          </div>
        </div>
